...
<div class="row coredocs-link">
	...
	<div class="col-md-6 docs-content">
		...
		<h3 id="{{ section_key }}-{{ link_key }}" class="coredocs-link-title">
			{{ link.title|default:link_key }} <a
				href="#{{ section_key }}-{{ link_key }}"><i class="fa fa-link"
		...
		<table class="parameters table table-bordered table-striped">
			...
			<tbody>
				...
				<tr>
					<td class="parameter-name"><code>{{ field.name }}</code>{% if
						field.required %} <span class="label label-warning">required</span>{%
						endif %}</td>
					<td>{% if field.schema.description %}{{
						field.schema.description }}{% endif %}</td>
				</tr>
				{% endfor %}
			</tbody>
		</table>
		...
		<p>The following parameters should be included as part of a URL
			query string.</p>
		<table class="parameters table table-bordered table-striped">
			<thead>
				<tr>
	...
	<div class="col-md-6 code-samples">{% if 'shell' in langs %}{%
		include "rest_framework/docs/langs/shell.html" %}{% endif %} {% if
		'python' in langs %}{% include "rest_framework/docs/langs/python.html"
		%}{% endif %} {% if 'javascript' in langs %}{% include
		"rest_framework/docs/langs/javascript.html" %}{% endif %}</div>
</div>
